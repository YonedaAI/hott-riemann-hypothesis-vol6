...
The transport step does not close. The upstream surface declares the carrier $\\texttt{burnolBlaschkeFactor.modelSpaceCarrier}$ via an opaque axiom and gives no introduction rule, no injection, and no identification. The paper therefore (i) closes every step of the pipeline that does not depend on the transport; (ii) states the precise typed proposition $\\texttt{OffCriticalDefectKernelBridge}$ that upgrades the conditional principal theorem $\\texttt{off\\_critical\\_zero\\_defect\\_kernel\\_of\\_bridge}$ to an unconditional one; and (iii) ships $\\texttt{Vol6.Obstruction.OffCriticalDefectKernelObstruction}$, a Lean module recording the exact content of the missing bridge. The Haskell artifact exhibits the singleton kernel-vector construction numerically at the test point $s = 0.6 + 14i$.`,
