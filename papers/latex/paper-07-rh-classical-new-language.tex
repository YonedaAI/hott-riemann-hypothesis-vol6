...
\title{Reducing the Riemann Hypothesis to a four-lemma No-Phantom Language \\
       \large Volume VI Synthesis \\
       \large (Paper 07)}
...
\begin{abstract}
If there were a non-trivial zero of the Riemann zeta function off the
critical line, it would create a nonzero vector in the Burnol/Blaschke
model space \(K_B = H^2 / B \cdot H^2\); a rational-dilation Yoneda
probe would detect that vector; and admissibility of the defect object
would forbid the resulting phantom---so no off-critical zero exists.
This is the No-Phantom Language argument for the Riemann Hypothesis.
